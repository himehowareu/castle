...
// @updateURL      https://ingress.wolflight.us/Arcs/arc.meta.js
// @downloadURL    https://ingress.wolflight.us/Arcs/arc.user.js
...
window.plugin.arcs.arcsparsing = function() {
    ...
    // Escape HTML characters in portal title to avoid 403 errors upon posting
    let arcList = JSON.parse(localStorage.getItem('arc_list'));
    for(let portal in arcList){
    	arcList[portal].startPortal.title = window.plugin.arcs.htmlEscape(arcList[portal].startPortal.title);
    	arcList[portal].endPortal.title = window.plugin.arcs.htmlEscape(arcList[portal].endPortal.title)
    }
    arcInput.value = JSON.stringify(arcList)
...
window.plugin.arcs.htmlEscape = function(string){
...
window.plugin.arcs.list = function() {
	...
	html +='Chat about Arcs here: <a target="_blank" href="https://t.me/joinchat/WL9VeHxmk08xOGYz">Telegram Link</a><br/>';

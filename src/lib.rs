...
pub fn run(_: &str, command: &&str) -> String {
...
pub fn run_lua(code: &str, _: &&str) -> String {
    let lua = Lua::new();
    lua.load(code)
        .eval::<String>()
...
pub fn file_to_base64(_: &str, file_path: &&str) -> String {
...
pub fn get_file(_: &str, url: &&str) -> String {
    let resp = reqwest::blocking::get(url.to_string()).expect("request failed");
    let body = resp.text().expect("body invalid");
    body
}

pub fn delete(_:&str,file_path:&&str)->String{
    fs::remove_file(file_path).unwrap();
    format!("deleted file {file_path}")
}

pub fn delete_folder(_:&str,file_path:&&str)->String{
